...
const VideoGames = () => {
  ...
  const VideoGames = products?.products?.filter(product => product.category === 'video games')
  ...
  return (
    <div className="mx-[5rem] my-[2.5rem]">
      <h1 className="bg-indigo-600 text-white text-xl font-bold flex items-center justify-between py-2 px-4 rounded-md shadow-md mb-4">
        Video Games <span className="text-sm text-gray-300 cursor-pointer">see all &gt;</span>
      </h1>
      ...
      >
        {VideoGames.map((product) => (
          <SwiperSlide key={product.id}>
            <VideoGamesItem product={product} />
          </SwiperSlide>
        ))}
      </Swiper>
    </div>
